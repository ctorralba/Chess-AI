...
State result(const State S, const Action A) {
	...
	if (result.last8Moves.size() >= 8) {
		//cout << "What is called in result: " << endl;
		/*for (int i = 0; i < result.last8Moves.size(); i++) {
			displayAction(result.last8Moves[i]);
			cout << endl;
		}*/
		//cout << "Deletion Occurs: " << endl;
	...
	case 'K':
		//White King
		//result.kRank = A.move.m_rank;
		//result.kFile = A.move.m_file;
	...
	if (S.turn == "White") {
		...
		for (int i = 0; i < result.black.size(); i++) {
			if (result.black[i].m_file == A.move.m_file && result.black[i].m_rank == A.move.m_rank) {
				///cout << "Removing: " << result.black[i].m_type << ": " << result.black[i].m_file << result.black[i].m_rank << endl;
				removeIndex = i;
				found = true;
				break;
			}
	...
	if (S.turn == "White"){
		for (int j = 0; j < result.black.size(); j++) {
			if (result.black[j].m_type == 'k') {
				...
				/*cout << "White is switching to black" << endl;
				cout << result.kFile << result.kRank << endl;*/
				break;
			}
		}
	}
		...
		for (int j = 0; j < result.white.size(); j++) {
			if (result.white[j].m_type == 'K') {
				...
				/*cout << "Black is switching to white" << endl;
				cout << result.kFile << result.kRank << endl;*/
				break;
			}
		}
...
bool State::draw() {
	...
	/*cout << "last8MovesActions: " << endl;
	for (int i = 0; i < last8Moves.size(); i++) {
		displayAction(last8Moves[i]);
		cout << endl;
	}*/
	//char wait;
	//cin >> wait;
	...
	///cout << "draw after last8moves: " << draw << endl;
	...
	///cout << "draw after 50 move rule: " << draw << endl;
	...
	///cout << "draw after insufficient material: " << draw << endl;
	return draw;
}

Action DLM(State S, int counter) {
	Action bestAction;
	///cout << "bestAction.utilityVal: " << bestAction.utilityVal << endl;
	bestAction = S.actionApplied;
	/*cout << "Call: ";
	cout << "DLM(";
	if (S.actionApplied.original.m_file != 'A'){
		displayAction(S.actionApplied);
	}
	else{
		cout << "Initial State";
	}
	...
	///	cout << "bestAction.utilityVal: " << bestAction.utilityVal << endl;
		...
		//Generating the list of actions for this state.
		...
		if (S.turn == "White") {
			for (int i = 0; i < S.black.size(); i++) {
				if (S.black[i].m_type == 'k') {
					oppkFile = S.black[i].m_file;
					oppkRank = S.black[i].m_rank;
					break;
				}
			}
		}
			...
			for (int i = 0; i < S.white.size(); i++) {
				if (S.white[i].m_type == 'K') {
					oppkFile = S.white[i].m_file;
					oppkRank = S.white[i].m_rank;
					break;
				}
			}
		}
		for (int i = 0; i < actionList.size(); i++) {
			if (actionList[i].move.m_rank == oppkRank && actionList[i].move.m_file == oppkFile) {
				win = true;
				break;
			}
		}
		if (!S.draw() && !win){
			...
			if (S.turn == S.max) {
				...
				///cout << "Max Frontier: " << endl;
				for (int i = 0; i < actionList.size(); i++) {
					frontier = actionList[i];
					eval = DLM(result(S, actionList[i]), counter - 1);
					frontier.utilityVal = eval.utilityVal;
					//temp is a placeholder
					//if the new value is higher than the previous value
					//set placeholder to the new value
					if (bestAction.utilityVal == -9999 || bestAction.utilityVal < frontier.utilityVal) {
						///cout << "There's a new higher value: ";
						bestAction = frontier;
				...
				///cout << "Min Frontier: " << endl;
				for (int i = 0; i < actionList.size(); i++) {
					frontier = actionList[i];
					eval = DLM(result(S, actionList[i]), counter - 1);
					frontier.utilityVal = eval.utilityVal;
					if (bestAction.utilityVal == -9999 || bestAction.utilityVal > frontier.utilityVal) {
						///cout << "There's a new lower value: ";
		...
		if (actionList.empty() || S.draw() || win) {
			cout << "Thinking...";
			if (actionList.empty()){
				cout << endl;
				cout << "ActionList: " << endl;
				for (int i = 0; i < actionList.size(); i++) {
					displayAction(actionList[i]);
					cout << endl;
				}
				displayState(S);
				cout << "Win: " << win << endl;
				if (S.max != S.turn){
					cout << "No moves?" << endl;
					//If opp of max king is getting attacked --differentiates stalemate
					char board[8][8];
					for (int i = 0; i < 8; i++) {
						for (int j = 0; j < 8; j++) {
							board[i][j] = '.';
					...
					cout << "I'm Losing?" << endl;
					bestAction.utilityVal = bestAction.utilityVal - 10;
				}
			}
			if (S.draw()) {
				cout << "I found a way to tie...";
				bestAction.utilityVal = bestAction.utilityVal - 5;
			}
			cout << endl;
			bestAction.utilityVal = evaluate(S) * 10;
		}
	}
	return bestAction;
}
...
Action IDM(State S, int finalDepth) {
	int counter = 0;
	bool terminal = false;
	Action act;
	S.max = S.turn;
	//Do I look for terminal states?
	while (/*!terminal ||*/ counter+1 <= finalDepth){
		counter++;
		///cout << "IDM Counter: ~~~~~~~~~~~~~~~~~~~~~~~~~~~" << counter << endl;
		act = DLM(S, counter);
		///cout << "IDM obtained action: " << endl;
		/*displayAction(act);
		cout << endl;*/
	}
	return act;
