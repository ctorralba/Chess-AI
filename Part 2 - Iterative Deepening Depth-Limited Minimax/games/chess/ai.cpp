...
bool AI::run_turn()
{
	...
	S.fiftyMoveRule = game->turns_to_draw;
	...
	if (game->moves.size() >= 8) {
		for (int i = 1; i <= 8; i++) {
			...
			/*cout << game->moves[game->moves.size() - i]->piece->owner->color << " did "
				<< game->moves[game->moves.size() - i]->piece->type << ":"
				<< game->moves[game->moves.size() - i]->from_file << game->moves[game->moves.size() - i]->from_rank << "->"
			    << game->moves[game->moves.size() - i]->to_file << game->moves[game->moves.size() - i]->to_rank << endl;*/
			//1 being the newest (first)
			//8 being the oldest (last iteration)
			S.last8Moves.push_back(prevMove);
		}

		/*cout << "Last 8 Moves: " << endl;
		for (int i = 0; i < S.last8Moves.size(); i++) {
			displayAction(S.last8Moves[i]);
			//so last i is the oldest
			cout << endl;
		}*/
	}
	...
	//vector<Action> mockup;
	actionList.clear();
	//S.actions(actionList);
	Action validAct = IDM(S, 4);
	//if (actionList.size() != 0) {
		//int randValid = rand() % actionList.size();
		//validAct = actionList[randValid];
	cout << "IDM used: " << endl;
