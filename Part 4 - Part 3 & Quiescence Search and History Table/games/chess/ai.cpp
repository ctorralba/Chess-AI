   ...
   if(game->moves.size() > 0)
	...
	if (game->moves.size() >= 8) {
		for (int i = 1; i <= 8; i++) {
			...
			if (game->moves[game->moves.size() - i]->captured != NULL) {
				prevMove.captureMove = true;
			}
	...
	//With this setup it exits around 4 to 5 (3 depth + 2 QS)depth against the difficulty of my own AI
	/////////////////////////////////
	//Call to IDM with timelimit and ABDLM
	///////////////////////////////////
	double wantedTime = 5000000000;
	Action validAct = IDM(S, 3, 2, wantedTime);

	cout << "QSHTABIDM used: " << endl;
