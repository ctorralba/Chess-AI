...
bool Mypiece:: operator==(const Mypiece &other) const {
	if (m_type == other.m_type && m_file == other.m_file && m_rank == other.m_rank) {
		return true;
	}
	else {
		return false;
...
bool Action:: operator==(const Action &other) const {
	...
	if (m_type == other.m_type) {
		if (original.m_type == other.original.m_type &&
			...
			move.m_rank == other.move.m_rank && captureMove == other.captureMove) {
			isEqual = true;
		}
	}
	return isEqual;
...
bool State:: operator==(const State &other) const {
	if (turn == other.turn && actionApplied == other.actionApplied && enPassant == other.enPassant && justCastled == other.justCastled &&
		castleK == other.castleK && castleQ == other.castleQ && castlek == other.castlek && castleq == other.castleq &&
		max == other.max && kRank == other.kRank && kFile == other.kFile && fiftyMoveRule == other.fiftyMoveRule &&
		white == other.white && black == other.black && last8Moves == other.last8Moves) {
		return true;
	}
	else {
		return false;
...
bool historyEntry:: operator==(const historyEntry &other) const {
	if (S == other.S && A == other.A && depth == other.depth) {
		return true;
	}
	else {
		return false;
...
bool Action::operator<(const Action &other) const {
	bool isGreater = false; //right is greater than left
	if (tableVal > other.tableVal) {
		isGreater = true;
	}
	return isGreater;
}
...
struct KeyHasher
{
	std::size_t operator()(const historyEntry& k) const
	{
		using std::size_t;
		using std::hash;
		using std::string;
		...
		//convert everything to a long string
		//then convert that string using hash<string>
		for (int i = 0; i < k.S.black.size(); i++) {
			data += k.S.black[i].m_type;
			data += to_string(k.S.black[i].m_rank);
			data += k.S.black[i].m_file;
		}
		for (int i = 0; i < k.S.white.size(); i++) {
			data += k.S.white[i].m_type;
			data += to_string(k.S.white[i].m_rank);
			data += k.S.white[i].m_file;
		}

		for (int i = 0; i < k.S.last8Moves.size(); i++) {
			data += k.S.last8Moves[i].original.m_type;
			data += to_string(k.S.last8Moves[i].original.m_rank);
			data += to_string(k.S.last8Moves[i].original.m_file);
			data += k.S.last8Moves[i].move.m_type;
			data += to_string(k.S.last8Moves[i].move.m_rank);
			data += to_string(k.S.last8Moves[i].move.m_file);
		}

		data += k.A.original.m_type;
		data += k.A.original.m_file;
		data += k.A.original.m_rank;
		data += k.A.move.m_type;
		data += k.A.move.m_file;
		data += k.A.move.m_rank;

		data += k.S.enPassant;
		data += k.S.turn;

		data += k.S.actionApplied.original.m_type;
		data += k.S.actionApplied.original.m_file;
		data += to_string(k.S.actionApplied.original.m_rank);
		data += k.S.actionApplied.move.m_type;
		data += k.S.actionApplied.move.m_file;
		data += to_string(k.S.actionApplied.move.m_rank);

		data += to_string(k.S.fiftyMoveRule);
		data += k.depth;

		/*return (((((((hash<char>()(k.A.original.m_type)
			^ (hash<char>()(k.A.original.m_file) << 1)) >> 1)
			^ (hash<int>()(k.A.original.m_file) << 1) >> 1)
			^ (hash<char>()(k.A.move.m_type) << 1) >> 1)
			^ (hash<char>()(k.A.move.m_file) << 1)) >> 1)
			^ (hash<int>()(k.A.move.m_file) << 1) >> 1)
			^ (hash<int>()(k.depth) << 1);*/
...
bool checkSpot(const int rank, const char file, const char board[][8], const string turn) {
	...
	if (turn == "White") {
		...
		if (rank == 5 && board[1][fileIndex(file)] == 'p') {
			if (rank == 4 && board[2][fileIndex(file)] == '.'){ //empty spot between the jump
				check = true;
			}
		}
		//Pawn diagonals
		...
		if (rank == 4 && board[6][fileIndex(file)] == 'P') {
			if (rank == 3 && board[5][fileIndex(file)] == '.') { //empty spot between the jump
				check = true;
			}
		}
		//Pawn diagonals
...
void displayState(const State S) {
	...
	cout << "Just Castled: " << S.justCastled << endl;
...
void displayHEntry(const historyEntry hE) {
	cout << "===============================" << endl;
	displayState(hE.S);
	displayAction(hE.A);
	cout << "Depth: " << hE.depth << endl;
	cout << "===============================" << endl;
}
State result(const State S, const Action A) {
	...
	//checking if the action is a castling move 
	if (S.max == "White") {
		if ((A.original.m_type == 'K' && A.original.m_file == 'e' && A.move.m_file == 'g' && A.original.m_rank == 1) ||
			(A.original.m_type == 'K' && A.original.m_file == 'e' && A.move.m_file == 'c' && A.original.m_rank == 1)) {
			result.justCastled = true;
		}
		else {
			result.justCastled = false;
		}
	}
	else {
		if ((A.original.m_type == 'k' && A.original.m_file == 'e' && A.move.m_file == 'g' && A.original.m_rank == 1) ||
...
int evaluate(State S) {
	...
	if (S.max == "White") {
		//Knight and Bishop away from center
		//Pawns should move up

		//Count white pieces
		for (int i = 0; i < S.white.size();i++) {
			switch (S.white[i].m_type) {
			case 'P':
				if (S.white[i].m_rank == 4 && (S.white[i].m_file == 'd' || S.white[i].m_file == 'e')) {
					eval = eval + 3;
				}
				eval = eval + 7;
				break;
			case 'N' :
			case 'B':
				//If position is ! at a, h, 8, 4 then + 4
				if (!(S.white[i].m_file == 'a' || S.white[i].m_file == 'h' || S.white[i].m_rank == 1 || S.white[i].m_rank == 8)) {
					eval = eval + 1;
				}
				eval = eval + 8;
				break;
		...
		for (int i = 0; i < S.black.size();i++) {
			switch (S.black[i].m_type) {
			case 'p':
				if (S.white[i].m_rank == 5 && (S.white[i].m_file == 'd' || S.white[i].m_file == 'e')) {
					eval = eval + 3;
				}
				eval = eval + 7;
				break;
			...
			case 'b' :
				//If position is ! at a, h, 8, 4 then + 4
				if (!(S.black[i].m_file == 'a' || S.black[i].m_file == 'h' || S.black[i].m_rank == 1 || S.black[i].m_rank == 8)) {
					eval = eval + 1;
				}
				eval = eval + 8;
				break;
	...
	if (black.size() <= 2 && white.size() <= 2) {
		...
		if (turn == "White") {
			...
			if (white.size() == 2 && black.size() == 2) {
				if ((black[0].m_type == 'b' || black[1].m_type == 'b') &&
					(white[0].m_type == 'B' || white[1].m_type == 'B')) {
					wIndex = 0;
					bIndex = 0;
					for (int i = 0; i < white.size(); i++) {
						if (white[i].m_type == 'B') {
...
bool evalQSState(const State& S) {
	bool callQS = false;
	//evaluate the state
	//if it's non-quiescent return true.
	//promotion
	//captures
	//check
	char wait;
	...
	for (int i = 0; i < S.white.size(); i++) {
		if (S.white[i].m_type == 'P' && S.white[i].m_rank == 7) {
			return true;
		}
	}
	for (int i = 0; i < S.black.size(); i++) {
		if (S.black[i].m_type == 'p' && S.black[i].m_rank == 2) {
			return true;
		}
	}

	//creating a board for checkspot
	char board[8][8];
	for (int i = 0; i < 8; i++) {
		for (int j = 0; j < 8; j++) {
			board[i][j] = '.';
	...
	//checking if any pieces of white are going to get captured
	for (int i = 0; i < S.white.size(); i++) {
		if (checkSpot(S.white[i].m_rank, S.white[i].m_file, board, "White")) {
			//A white piece is in "check"
			return true;
		}
	}
	for (int i = 0; i < S.black.size(); i++) {
		if (checkSpot(S.black[i].m_rank, S.black[i].m_file, board, "Black")) {
			//A black piece is in "check"
			return true;
		}
	}

	//checking if the king is in check for current player
	if (checkSpot(S.kRank, S.kFile, board, S.turn)) {
		//The turn piece is now in "check"
		return true;
	}
...
Action QSHTABDLM(State S, int counter, int qsCounter, int& alpha, int& beta, unordered_map <historyEntry, int, KeyHasher>& historyTable) {
	Action bestAction;
	bestAction = S.actionApplied;
	//conditions for evaluation:
	//1. If it is Q and the counter is @ 0
	//2. If it is a non-Q and the counter is @ 0 and the qsCounter is @ 0
	
	//Determining if state is QS
	bool qsState = false;
	qsState = evalQSState(S);

	if (counter == 0 && (!qsState || (qsState && qsCounter == 0))) { //Base Case
		...
		if (!(S.draw() || actionList.empty())){ //if state is a draw or win don't recurse
			...
			//Put history table values in the actionList
			bool needSort = false;
			for (int i = 0; i < actionList.size(); i++) {
				//for each action lookup in the history table
				//by creating an entry
				historyEntry lookup;
				lookup.S = S;
				lookup.A = actionList[i];
				lookup.depth = counter;
				actionList[i].tableVal = historyTable[lookup];
				if (actionList[i].tableVal > 0) {
					needSort = true;
				}
			}
			//Reorder elements in a actions based on the history table labels >;
			if (needSort){
			...
			if (S.turn == S.max) {
				...
				for (int i = 0; i < actionList.size(); i++) {
					...
					int betaState = beta;
					if (qsState && counter == 0) {
						eval = QSHTABDLM(result(S, actionList[i]), 0, qsCounter-1, alpha, beta, historyTable);
					}
					else{
						eval = QSHTABDLM(result(S, actionList[i]), counter - 1, qsCounter, alpha, beta,historyTable);
					}
				...
				}//Updating the history table after checking the frontier or pruning
				historyEntry newValue;
				newValue.S = S;
				newValue.A = bestAction;
				newValue.depth = counter;
				...
				newValue.A = bestAction;
				newValue.depth = counter;
				//Inserting into history table (after prune or best action)
				historyTable[newValue]++;
			}
		}
		else{
			...
			if (actionList.empty()){
				...
				if (checkSpot(S.kRank, S.kFile, board, S.turn)){
					if (S.max == S.turn) { //loss
						bestAction.utilityVal = bestAction.utilityVal - 999;
					}
					else { //win
						bestAction.utilityVal = bestAction.utilityVal + 999;
					}
				}//else = stalemate draw
			}//else = regular draw (other than stalemate)
...
//wantedTime is in ns
Action IDM(State S, int finalDepth, int qsDepth, const double wantedTime) {
	//Initialization
	int counter = 0;
	bool terminal = false;
	Action act;
	S.max = S.turn;
	int alpha, beta;
	//For a specific state where the history table value is stored in the action
	unordered_map <historyEntry, int, KeyHasher> historyTable;
