...
struct Mypiece {
	...
	bool operator==(const Mypiece &other) const;
};
...
struct Action {
	...
	int tableVal; //this is tied to the state.
				  
	//In the function it is sorted by descending order 3, 1, 0, 0
	bool operator<(const Action &other) const;
...
struct State {
	...
	string enPassant = "-";
	//Castling:
	bool justCastled = false; //for max player
	bool castleK = false;
	bool castleQ = false;
	bool castlek = false;
	bool castleq = false;
