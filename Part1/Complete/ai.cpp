...
bool AI::run_turn()
{
	...
	if (player->color == "White") {
		std::cout << "Testing White " << std::endl;
	}
	else {
		std::cout << "Testing Black" << std::endl;
	}
	//file = vertical
	//rank = row
	//read in and fill board - State
	//void ACTIONS(State, vector<Action>* v)
	//ACTIONS(State, &A)
	//NEW_STATE = RESULT(State, Action[random])
	...
	//fen = "1nbk2nr/r1pp1p2/p2bpB1p/1p4P1/1PP5/1Q3NP1/P2PPPB1/RN2K2R b KQ -";
	cout << fen << endl;
	int rank = 8;
	char file = 'a';
	int spaceCount = 0;
	...
	for (int i = 0; i < S.white.size(); i++) {
		std::cout << S.white[i].m_file << S.white[i].m_rank << ": " << S.white[i].m_type << endl;
	}

	std::cout << "Black Pieces: " << std::endl;
	for (int i = 0; i < S.black.size(); i++) {
		std::cout << S.black[i].m_file << S.black[i].m_rank << ": " << S.black[i].m_type << endl;
	}*/
	...
	cout << "Valido Action: " << validAct.original.m_type << ": " << validAct.original.m_file << validAct.original.m_rank << " moved " << validAct.move.m_file << validAct.move.m_rank << endl;
