...
bool enPassantCheck(const char board[][8], string enPassant, const string opp, const int rank, const int file) {
	...
	if (opp == "White"){
		if (file == enPassFile && rank == enPassRank-1) {
...
bool checkSpot(const int rank, const char file, const char board[][8], const string turn) {
	...
	for (int i = 0; i < 8; i++) {
		...
		while (emptyBound(board, rank + rmod, file + fmod) == 'e') {
			switch (i) {
			case 0:
				fmod++;
				break;
			...
			if (turn == "White") {
				if (emptyBound(board, rank + rmod, file + fmod) != 'n') {
					if ((emptyBound(board, rank + rmod, file + fmod) == 'q'
						|| emptyBound(board, rank + rmod, file + fmod) == 'r') && (i < 4)) {
						check = true;
					}
					if ((emptyBound(board, rank + rmod, file + fmod) == 'q'
						|| emptyBound(board, rank + rmod, file + fmod) == 'b') && (i >= 4)) {
						check = true;
					}
				}
			}
	...
	if (turn == "White") {
		//Pawn jump
		if (rank == 5 && board[2][fileIndex(file)] == 'p') {
			check = true;
		}
		//Pawn diagonals
		...
		//Pawn jump
		if (rank == 4 && board[2][fileIndex(file)] == 'P') {
			check = true;
		}
		//Pawn diagonals
		if (emptyBound(board, rank - 1, file - 1) == 'P') {
...
bool emptyOrOpp(const char board[][8], const string opp, const int rank, const char file) {
	bool valid = false;
	...
	if (checkSpot(rank, file, board, opp)) {
		valid = false;
	}
	//true if valid
	return valid;
...
void /*vector<Action>*/ State::actions() { //ACTIONS FUNCTION FOR STATE
	...
	//do opponent of turn first (used to determine check)
	if (turn == "White") {
		turnSize = white.size();
		player = white;
	else {
	...
	for (int i = 0; i < turnSize; i++) {
		...
		case ('P'):
			...
			if (player[i].m_rank == 2) {
				...
				//board[rank][file]
				//check jump 2
				if (board[6][fileIndex(player[i].m_file)] == '.') {
					if (emptyOrOpp(board, turn, 4, player[i].m_file)) {
			...
			//check jump 1
			//Moving Up
			if (emptyOrOpp(board, turn, player[i].m_rank + 1, player[i].m_file))) {
		...
		case('p'):
			if (player[i].m_rank == 7) {
				...
				if (board[2][fileIndex(player[i].m_file)] == '.') {
					if (emptyOrOpp(board, turn, 5, player[i].m_file)) {
						tmp.setPiece('p', player[i].m_file, player[i].m_rank - 2);
						valid.setAction('p', player[i], tmp);
						actionList.push_back(valid);
					}
		...
		case('q'):
			...
			for (int k = 0; k < pieceDirections; k++) {
				...
				while (emptyOrOpp(board, turn, player[i].m_rank + rmodifier, player[i].m_file + fmodifier) && !enemyEncounter) {
					if (turn == "White") {
						if (piece == 'R') {
		...
		case ('n'):
			...
			for (int k = -2; k <= 2; k++) {
				if (k != 0) {
					if (k % 2 == 0) {
						if (emptyOrOpp(board, turn, player[i].m_rank + k, player[i].m_file + 1)) {
							if (turn == "White") {
						...
						if (emptyOrOpp(board, turn, player[i].m_rank + k, player[i].m_file + 2)) {
							if (turn == "White") {
		...
		case ('k'):
			...
			for (int k = -1; k <= 1; k++) {
				for (int j = -1; j <= 1; j++) {
					if (emptyOrOpp(board, turn, player[i].m_rank + k, player[i].m_file + j))
						&& !attackSpot(kRank, kFile, oppActions){
						if (turn == "White") {
