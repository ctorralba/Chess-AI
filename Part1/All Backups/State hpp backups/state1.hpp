...
bool checkSpot(const int rank, const char file, const char board[][8], const string turn) {
	...
	for (int i = 0; i < 8; i++) {
		...
		if (turn == "White") {
			...
			if (emptyBound(board, rank, file) != 'w') {
				if (i < 4){
					if (emptyBound(board, rmod, fmod) == 'Q'
						|| emptyBound(board, rmod, fmod) == 'R') {
						check = true;
...
bool emptyOrOpp(const char board[][8], const string opp, const int rank, const char file, const int kRank, const char kFile) {
	bool valid = false;
	...
	//cout << "b4 checkspot: " << valid << endl;
	/*if (!checkSpot(kRank, kFile, board, opp)) {
		valid = true;
	}*/
	//cout << "after checkspot: " << valid << endl;
	//true if valid
	return valid;
}
	...
	for (int i = 0; i < turnSize; i++) {
		...
		case ('P'):
			...
			if (player[i].m_rank == 2) {
				...
				if (board[5][fileIndex(player[i].m_file)] == '.') {
					if (emptyOrOpp(board, turn, 4, player[i].m_file, kRank, kFile)) {
			...
			//check jump 1
			//Moving Up
			if (emptyOrOpp(board, turn, player[i].m_rank + 1, player[i].m_file, kRank, kFile)) {
		...
		case('p'):
			if (player[i].m_rank == 7) {
				...
				if (board[2][fileIndex(player[i].m_file)] == '.') {
					if (emptyOrOpp(board, turn, 5, player[i].m_file, kRank, kFile)) {
						tmp.setPiece('p', player[i].m_file, player[i].m_rank - 2);
						valid.setAction('p', player[i], tmp);
						actionList.push_back(valid);
					}
		...
		case('q'):
			...
			for (int k = 0; k < pieceDirections; k++) {
				...
				while (emptyOrOpp(board, turn, player[i].m_rank + rmodifier, player[i].m_file + fmodifier, kRank, kFile) && !enemyEncounter) {
					if (turn == "White") {
						if (piece == 'R') {
		...
		case ('n'):
			...
			for (int k = -2; k <= 2; k++) {
				if (k != 0) {
					if (k % 2 == 0) {
						if (emptyOrOpp(board, turn, player[i].m_rank + k, player[i].m_file + 1, kRank, kFile)) {
							if (turn == "White") {
						...
						if (emptyOrOpp(board, turn, player[i].m_rank + k, player[i].m_file + 2, kRank, kFile)) {
							if (turn == "White") {
			...
			for (int k = -1; k <= 1; k++) {
				for (int j = -1; j <= 1; j++) {
					if (!(k==0 && j==0)){
						...
						if (emptyOrOpp(board, turn, player[i].m_rank + k, player[i].m_file + j, kRank, kFile)){
							//if not in check
							//so it should be 0
